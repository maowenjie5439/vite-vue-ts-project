    ...
    <div>
        <el-form :inline="true" class="search-form" :model="searchData">
            ...
            <el-form-item label="用户角色">
                <el-select v-model="searchData.role" class="m-2" size="large" placeholder="请选择用户角色" style="width: 100px;">
...
const handleSearchUser = () => {
    ...
    if (searchData.nickName || searchData.role) {
        ...
        if (searchData.role) {
            ...
            res = res.filter((item) => {
                return item.role.some((value) => {
                    if(searchData.role === 0) {
                        return true
                    }
...
const ensureEditUser = () => {
    console.log('确认修改editUser: ', editUser);
...
const fetchUserList = async () => {
    const res = await get({}, '/getUserList', searchData)
    if(constants.isSuccess(res.code)) {
        userList.value = res.data || []
    }
};
const fetchRoleList = async () => {
    const res = await get({}, '/getRoleList', searchData)
    if(constants.isSuccess(res.code)) {
